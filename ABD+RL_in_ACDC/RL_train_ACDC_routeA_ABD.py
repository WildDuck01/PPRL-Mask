...
# -------------------------
# RL Route-A (contextual bandit) imports
# -------------------------
from collections import defaultdict
from bcp_rl_routeA_utils import (
    MaskActionConfig,
    generate_mask_by_action,
)
from rl_agent_bandit_routeA import (
    BanditConfig,
    ContextualBanditAgent,
    extract_state_from_encoder_feature,
)
...
# -------- RL Route-A (optional) --------
parser.add_argument('--use_rl', type=int, default=1, help='whether to use RL Route-A for mask selection (1=yes, 0=no)')
parser.add_argument('--rl_warmup_iters', type=int, default=1000, help='warm-up iters to use random mask before RL takes over')
parser.add_argument('--rl_reward_clip', type=float, default=0.2, help='clip advantage-based reward into [-clip, clip]')
parser.add_argument('--rl_reward_scale', type=float, default=5.0, help='scale clipped reward')
parser.add_argument('--rl_ema_momentum', type=float, default=0.99, help='EMA momentum for reward baseline')
parser.add_argument('--rl_lr', type=float, default=1e-4, help='RL policy network learning rate')
parser.add_argument('--rl_log_interval', type=int, default=200, help='print RL stats every N iters')
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    # model_1 returns encoder feature for RL state extraction (C=256 => state_dim=258)
    model_1 = BCP_net(in_chns=1, class_num=num_classes, returnEncoderFeature=True)
    ...
    # -------------------------
    # RL Route-A (contextual bandit) setup
    # -------------------------
    # When labeled set is very small, pseudo labels are noisy early on; extend warm-up to reduce instability.
    rl_warmup_iters = int(args.rl_warmup_iters)
    if args.labelnum <= 7:
        rl_warmup_iters = max(rl_warmup_iters, 5000)

    bandit_cfg = BanditConfig(state_dim=258, action_dim=4, lr=float(args.rl_lr))
    rl_agent = ContextualBanditAgent(bandit_cfg, seed=args.seed + 1)
    mask_cfg = MaskActionConfig(beta=2.0/3.0, guided_beta=0.50, topk_percent=0.02, per_sample=True)
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                plab_a_s = get_ACDC_masks(pre_a_s, nms=1)  # plab_a.shape=[B, H, W]
                plab_b_s = get_ACDC_masks(pre_b_s, nms=1)

            # -------------------------
            # RL Route-A mask selection (or warm-up random mask)
            # -------------------------
            action = 0
            state = None
            if int(args.use_rl) == 1 and iter_num >= rl_warmup_iters:
                # Build state from model_1 encoder feature + teacher uncertainty/disagreement on uimg_a
                model1_was_training = model_1.training
                model_1.eval()
                with torch.no_grad():
                    # teacher probs for uimg_a
                    pT = torch.softmax(pre_a, dim=1)
                    ent_map = -(torch.clamp(pT, 1e-6, 1.0) * torch.log(torch.clamp(pT, 1e-6, 1.0))).sum(dim=1)
                    ent_mean = float(ent_map.mean().item())

                    # student logits + feature for uimg_a
                    s_logits_u, feat_u = model_1(uimg_a)
                    pS = torch.softmax(s_logits_u, dim=1)
                    dis_mean = float((pT * (torch.log(torch.clamp(pT, 1e-6, 1.0)) - torch.log(torch.clamp(pS, 1e-6, 1.0)))).sum(dim=1).mean().item())

                    state = extract_state_from_encoder_feature(feat_u, ent_mean, dis_mean)
                    action = rl_agent.choose_action(state, train=True)
                    action_counts[action] += 1

                    # Mask for outward branch (uimg_a as base)
                    M_out = generate_mask_by_action(
                        action,
                        teacher_logits=pre_a,
                        student_logits=s_logits_u if action == 2 else None,
                        cfg=mask_cfg,
                    )  # [B,H,W] in {0,1}

                    # Mask for inward branch (img_b as base)
                    t_logits_in = ema_model(img_b)
                    s_logits_in, _ = model_1(img_b)
                    M_in = generate_mask_by_action(
                        action,
                        teacher_logits=t_logits_in,
                        student_logits=s_logits_in if action == 2 else None,
                        cfg=mask_cfg,
                    )
                ...
                loss_mask_out = M_out.float()
                loss_mask_in = M_in.float()
            else:
                # warm-up: original random rectangle mask (single rect for the batch)
                _, loss_mask = generate_mask(img_a)
                loss_mask_out = loss_mask.float()
                loss_mask_in = loss_mask.float()
            consistency_weight = get_current_consistency_weight(iter_num//150)

            # Broadcast masks to image tensors: [B,1,H,W]
            img_mask_out_img = loss_mask_out.unsqueeze(1).to(dtype=volume_batch.dtype)
            img_mask_in_img = loss_mask_in.unsqueeze(1).to(dtype=volume_batch.dtype)

            net_input_unl_1 = uimg_a * img_mask_out_img + img_a * (1.0 - img_mask_out_img)
            net_input_l_1 = img_b * img_mask_in_img + uimg_b * (1.0 - img_mask_in_img)
            net_input_1 = torch.cat([net_input_unl_1, net_input_l_1], dim=0) 

            net_input_unl_2 = uimg_a_s * img_mask_out_img + img_a_s * (1.0 - img_mask_out_img)
            net_input_l_2 = img_b_s * img_mask_in_img + uimg_b_s * (1.0 - img_mask_in_img)
            net_input_2 = torch.cat([net_input_unl_2, net_input_l_2], dim=0)

            # Model1 Loss
            out_unl_1, _ = model_1(net_input_unl_1)
            out_l_1, _ = model_1(net_input_l_1)
            ...
            unl_dice_1, unl_ce_1 = mix_loss(out_unl_1, plab_a, lab_a, loss_mask_out, u_weight=args.u_weight, unlab=True)
            l_dice_1, l_ce_1 = mix_loss(out_l_1, lab_b, plab_b, loss_mask_in, u_weight=args.u_weight)
            ...
            unl_dice_2, unl_ce_2 = mix_loss(out_unl_2, plab_a_s, lab_a_s, loss_mask_out, u_weight=args.u_weight, unlab=True)
            l_dice_2, l_ce_2 = mix_loss(out_l_2, lab_b_s, plab_b_s, loss_mask_in, u_weight=args.u_weight)
            ...
            # -------------------------
            # RL Route-A learning step (reward regression)
            # -------------------------
            if int(args.use_rl) == 1 and iter_num >= rl_warmup_iters and state is not None:
                with torch.no_grad():
                    score = -float(loss.detach().item())
                    adv = score - ema_score
                    ema_score = float(args.rl_ema_momentum) * ema_score + (1.0 - float(args.rl_ema_momentum)) * score
                    reward = float(np.clip(adv, -float(args.rl_reward_clip), float(args.rl_reward_clip)) * float(args.rl_reward_scale))
                    action_reward_sum[action] += reward

                    # next_state: recompute on uimg_a after update (dropout disabled)
                    model1_was_training2 = model_1.training
                    model_1.eval()
                    s_logits_u2, feat_u2 = model_1(uimg_a)
                    pT2 = torch.softmax(pre_a, dim=1)
                    ent_map2 = -(torch.clamp(pT2, 1e-6, 1.0) * torch.log(torch.clamp(pT2, 1e-6, 1.0))).sum(dim=1)
                    ent_mean2 = float(ent_map2.mean().item())
                    pS2 = torch.softmax(s_logits_u2, dim=1)
                    dis_mean2 = float((pT2 * (torch.log(torch.clamp(pT2, 1e-6, 1.0)) - torch.log(torch.clamp(pS2, 1e-6, 1.0)))).sum(dim=1).mean().item())
                    next_state = extract_state_from_encoder_feature(feat_u2, ent_mean2, dis_mean2)
                    if model1_was_training2:
                        model_1.train()

                rl_agent.store_transition(state, int(action), float(reward), next_state)
                rl_loss = rl_agent.update(batch_size=32)
                ...
                if iter_num % int(args.rl_log_interval) == 0:
                    # print a small RL summary
                    total = max(1, sum(action_counts.values()))
                    action_dist = [round(action_counts[i] / total, 4) for i in range(4)]
                    avg_reward = [round(action_reward_sum[i] / max(1, action_counts[i]), 6) for i in range(4)]
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                # For validation, disable feature return so `test_single_volume` can call the model normally.
                _ret_feat = getattr(model_1, 'returnEncoderFeature', None)
                if _ret_feat is not None:
                    model_1.returnEncoderFeature = False
                model_1.eval()
                metric_list = 0.0
                for _, sampled_batch in enumerate(valloader):
                ...
                model_1.train()
                if _ret_feat is not None:
                    model_1.returnEncoderFeature = True
...
if __name__ == "__main__":
    ...
    shutil.copy('./code/RL_train_ACDC_BCP_routeA_ABD.py', self_snapshot_path)
